...
def get_context():
    # Combine both command and response for more context
    return " ".join([f"User: {item['command']} | AI: {item['response']}" for item in context_history])
...
def respond_gui(response, lang='en'):
    output_text.configure(state="normal")
    output_text.insert(ctk.END, "Alex: " + response + "\n")
    output_text.configure(state="disabled")
    output_text.see(ctk.END)
    ...
    with mic as source:
        ...
        while True:
            ...
            try:
                ...
                if "alex" in speech_text.lower():
                    print("Wake word detected! Activating chatbot...")
                    activate_chatbot()
...
def activate_chatbot():
    ...
    print("Chatbot is now active. How can I assist you?")
...
def update_personal_context(command, response):
    if "my name is" in command.lower():
        user_name = command.split("my name is")[-1].strip()
        context_history.append({"user_name": user_name})
    # Store more personal details based on the interaction
...
def start_listening():
    # Start listening for the wake word in a new thread
    listening_thread = threading.Thread(target=listen_for_wake_word, daemon=True)
...
app = ctk.CTk()
app.geometry("650x400")  # Adjusted size for better layout
app.title("MMU Innov8 Lab AI Assistant")

# Title Label
title_label = ctk.CTkLabel(app, text="MMU Innov8 Lab AI Assistant (ALEX)", font=("Arial", 20, "bold"))
title_label.pack(pady=(10, 5))  # Adding some padding around the title

# Create a frame for better organization
frame = ctk.CTkFrame(app)
frame.pack(pady=20, padx=20, fill="both", expand=True)

# Output Text Box (adjusted for cleaner alignment)
output_text = ctk.CTkTextbox(frame, width=560, height=220)
output_text.grid(row=0, column=0, columnspan=2, padx=20, pady=(10, 5), sticky="nsew")
output_text.configure(state="disabled")  # Start as disabled

# Command Entry (centered, placed below the text box)
command_entry = ctk.CTkEntry(frame, width=500, placeholder_text="Type your command here...")
command_entry.grid(row=1, column=0, padx=(20, 5), pady=(5, 10), sticky="w")

# Microphone Button (adjusted to fit beside the entry)
audio_button = ctk.CTkButton(frame, text="🎤", command=start_listening, width=40)
audio_button.grid(row=1, column=1, padx=(5, 20), pady=(5, 10), sticky="e")

# Function to handle the Enter key press
def on_enter(event):
    listen_for_command_gui()

# Bind the Enter key to the command entry box
